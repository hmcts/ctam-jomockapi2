...
- [ ] T020 Review `HealthcheckController` (T010) to confirm no exception is caught/swallowed there, so an unexpected internal failure falls through to Spring Boot's default error handling instead of a forced `200` (spec.md EC-003, FR-009). This is a documented manual-review check, not an automated test: the endpoint has no dependency or fault-injection seam to trigger a real internal failure against, and plan.md's Complexity Tracking defers building the constitution's `GlobalExceptionHandler`/`ErrorResponse` (Principle IX) to a future foundation feature — this review only confirms nothing in this feature's own code overrides Spring's default behaviour in the meantime.
...
- **User Story 2 (Phase 4)**: Depends on T009/T010 (US1's `HealthResponse`/controller) existing; adds documentation/annotations on top of it — does not require US1's tests to be modified.
...
- **US2 (P2)**: Builds on the same controller/record US1 creates (T009, T010) but adds only annotations/tests — does not change US1's behaviour or tests.
...
- **T005 (`.editorconfig`) added 2026-09-18** in response to `/speckit-analyze` finding K1, and every task from T006 onward renumbered by one accordingly. Unlike the IX/XIII/XIV deferrals, this constitution requirement (Technology & Architecture Constraints) is cheap enough (one static file, no dependency/build/CI change) that it's implemented now rather than deferred.
