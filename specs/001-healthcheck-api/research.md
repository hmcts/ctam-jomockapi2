...
## Decision: Base package `uk.gov.hmcts.ctam.elinks`, organised by layer

**Rationale**: Constitution Principle II now mandates an HMCTS-style base package following the pattern `uk.gov.hmcts.<domain-code>`, organised by layer (`controllers/`, `services/`, `repository/`, `mappers/`, `domain/`, `entity/`, `exceptions/`, `filters/`, `config/`) rather than by feature. `elinks` is the domain code for this mock (the E-Links API); `ctam` is inserted ahead of it to group the package under this repository/programme's own code (`ctam-jomockapi2`), giving `uk.gov.hmcts.ctam.elinks`. This is a two-segment domain code rather than the constitution's single-segment example (`uk.gov.hmcts.cp`) — an intentional, requested deviation for this repository, not a misapplication of the pattern. This supersedes this feature's original `uk.gov.hmcts.reform.ctam.jomockapi` / package-by-feature (`healthcheck/`) decision, made before the constitution was amended to specify this.
