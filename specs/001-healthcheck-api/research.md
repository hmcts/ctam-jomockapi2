...
## Decision: Base package `uk.gov.hmcts.ctam.jo`, organised by layer

**Rationale**: Constitution Principle II now mandates an HMCTS-style base package following the pattern `uk.gov.hmcts.<domain-code>`, organised by layer (`controllers/`, `services/`, `repository/`, `mappers/`, `domain/`, `entity/`, `exceptions/`, `filters/`, `config/`) rather than by feature. `jo` is the domain code for this mock (Judicial Office, matching the `joh-elinks-api/` naming already used elsewhere in this repository); `ctam` is inserted ahead of it to group the package under this repository/programme's own code (`ctam-jomockapi2`), giving `uk.gov.hmcts.ctam.jo`. This is a two-segment domain code rather than the constitution's illustrative single-segment example (`uk.gov.hmcts.cp`) — an intentional, requested deviation for this repository, documented here per Governance's deviation-approval requirement (see plan.md's Constitution Check, Principle II row, for the corresponding caveat). This supersedes this feature's original `uk.gov.hmcts.reform.ctam.jomockapi` / package-by-feature (`healthcheck/`) decision, made before the constitution was amended to specify this, and its own immediately prior `uk.gov.hmcts.ctam.elinks` choice (renamed to `jo` per explicit request).
...
**Rationale**: Satisfies FR-010 by generating the Swagger/OpenAPI document from annotations on the controller method (`@Operation`, `@ApiResponse`) and the `HealthResponse` record (as the declared response type) rather than hand-maintaining a separate spec file that can drift from the code (reinforces Principle I, contract fidelity, for this and every future endpoint). Unaffected by the constitution amendments beyond the response now having a real schema to generate. The `@GetMapping` also declares `produces = MediaType.APPLICATION_JSON_VALUE`: without it, springdoc documents the `200` response's content type as the generic `*/*` (Spring's default when no explicit media type is negotiated) rather than `application/json`, which would not match contracts/healthcheck.md's documented shape — discovered while implementing T015 (found by `/speckit-converge`, finding F2).
...
**Rationale**: Constitution Principle XI requires unit, controller/API, and contract test coverage. For an endpoint with no business logic, "unit" and "controller" coverage collapse into one `@WebMvcTest`/MockMvc test class (success `200` with `{"status":"ok"}` body, no-input success, non-`GET` method rejected — spec.md AC-001, AC-002, AC-003, SC-005). A `@SpringBootTest` smoke test proves the full application context wires the endpoint end-to-end. "Contract" coverage is satisfied by asserting the generated `/v3/api-docs` payload documents `GET /api/v1/healthcheck` → `200` "Service is healthy" with the `status` field schema (FR-010, AC-006), since no separate machine-readable source contract file exists in this repo to diff against (only the binary `joh-elinks-api/swagger-ui-elinks-api-v5.pdf`).

**Dependency note (added after implementation, found by `/speckit-converge`, finding F1)**: Spring Boot 4.1.1 splits this test tooling across more Gradle dependencies than earlier versions assumed. `spring-boot-starter-test` alone no longer provides `@WebMvcTest` — it now lives in `spring-boot-starter-webmvc-test`, a separate dependency added to `build.gradle` for T006. That same dependency transitively pulls in `spring-boot-resttestclient` (which hosts the relocated `TestRestTemplate`, at `org.springframework.boot.resttestclient.TestRestTemplate`), but `TestRestTemplate`'s constructor additionally requires two more modules not pulled in automatically: `spring-boot-http-client` (`ClientHttpRequestFactoryBuilder`) and `spring-boot-restclient` (`RestTemplateBuilder`), both added explicitly to `build.gradle` while implementing T010's smoke-test verification.
