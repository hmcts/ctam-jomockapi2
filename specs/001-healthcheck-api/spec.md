...
1. **AC-001**: **Given** the application is running and able to serve HTTP traffic, **When** a monitoring component sends `GET /api/v5/healthcheck`, **Then** the response is `HTTP 200 OK` with the documented description "Service is healthy".
...
2. **AC-006**: **Given** the published API documentation, **When** the healthcheck endpoint is viewed, **Then** it is documented as `GET /api/v5/healthcheck` returning `200` with description "Service is healthy", and no additional response schema is implied beyond what is documented.
...
1. **AC-003**: **Given** the healthcheck endpoint exists at `/api/v5/healthcheck`, **When** a non-`GET` request (e.g., `POST`) is sent to that path, **Then** the healthcheck success response is not returned, and the application's standard handling for unsupported methods on that path applies.
...
- **FR-001**: System MUST expose an endpoint reachable via `GET /api/v5/healthcheck`.
...
- **FR-010**: The healthcheck capability MUST be represented in the application's published API documentation as `GET /api/v5/healthcheck` with a `200` response described as "Service is healthy" and a response schema limited to the `status` field (FR-002), without introducing any additional response schema beyond that.
...
- Any base path or context path outside of `/api/v5/healthcheck` itself is handled by existing application/infrastructure routing configuration and is not altered by this feature.
