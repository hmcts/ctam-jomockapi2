...
**Testing**: JUnit 5 + Spring's `@WebMvcTest`/`MockMvc` for controller behaviour, `@SpringBootTest` for a thin end-to-end smoke test, and a lightweight assertion against the generated `/v3/api-docs` payload as the "contract" test (Principle XI) — see research.md for rationale. Spring Boot 4.1.1 splits this tooling across more test-scoped Gradle dependencies than earlier versions: `spring-boot-starter-test` alone no longer carries `@WebMvcTest` (moved to `spring-boot-starter-webmvc-test`, which also transitively supplies `TestRestTemplate`'s two further runtime dependencies, `spring-boot-http-client` and `spring-boot-restclient`) — discovered while implementing T006/T007 and added to `build.gradle` alongside `spring-boot-starter-test` (found by `/speckit-converge`, finding F1).
...
| II. Java Spring Boot Architecture | **Pass, with one documented deviation.** Implemented as Java 25 + Spring Boot 4.1.1 + Gradle (per the amended principle's exact pins); package-by-layer structure (`controllers/`, `domain/`); controller stays thin (no business logic exists to extract); health-endpoint response convention followed (`HealthResponse` record, `{"status":"ok"}`). Base package `uk.gov.hmcts.ctam.jo` uses a two-segment domain code (`ctam.jo`) rather than the principle's illustrative single-segment example (`uk.gov.hmcts.cp`) — an intentional, requested deviation, reasoned through in research.md's Decision and reconfirmed here as agreed before implementation, per Governance (found by `/speckit-analyze`, finding F1). |
...
| IX. Centralised Validation and Error Handling | **Partial — deferred, documented below.** No `GlobalExceptionHandler`/`ErrorResponse` is added by this feature; Spring Boot's default error handling (not yet the constitution's full shared error shape) covers the unexpected-failure edge case (spec.md EC-003) for now. The log-severity clause (4xx→warning, 5xx→error) is deferred for the same reason: this endpoint emits no custom log line for its 405 case — Spring's default framework handling, not application-logged — so there is nothing yet for severity-appropriate logging to apply to; see Complexity Tracking below. |
...
| XVII. API Testability and Postman Artifacts | **Pass** (added to the constitution in v1.9.0, after this feature's initial implementation — closed by T024). A Postman collection and environment file exist at `/postman/ctam-jomockapi.postman_collection.json` / `.postman_environment.json`, with a request for `GET /api/v1/healthcheck` using the `{{baseUrl}}` environment variable and an embedded test script asserting `200` status, `application/json` content type, and body `{"status":"ok"}`. Verified to actually exercise the running API via `npx newman run` against a live instance — all 3 assertions passed (found by `/speckit-analyze`/`/speckit-converge`, finding D2/F1). |
| XVIII. API Contract and Swagger/OpenAPI Requirements | **Pass** (added to the constitution in v1.10.0, after this feature's initial implementation — closed by T025). The endpoint/response level was already fully documented via `@Operation`/`@ApiResponse` (T015); the one gap — the `status` field carrying no description — is closed by a `@Schema(description = ...)` annotation on the `HealthResponse` record component, verified present in the generated `/v3/api-docs` output (found by `/speckit-analyze`/`/speckit-converge`, finding D3/F2). |
...
**Re-evaluated again after constitution v1.8.1 → v1.9.0 → v1.10.0** (2026-09-22, in response to `/speckit-analyze` findings D1/D2/D3 and `/speckit-converge` findings F1/F2/F3): two new principles were added after this feature's implementation had already completed — XVII (API Testability and Postman Artifacts) and XVIII (API Contract and Swagger/OpenAPI Requirements). Both initially assessed as failing/partial for this feature (no Postman artifacts existed; the `status` field's OpenAPI schema had no description), since neither existed when T001–T021 were implemented. Both gaps are now closed: T024 added a Postman collection and environment file (verified via `npx newman run` against a live instance), and T025 added a `@Schema` description to `HealthResponse.status` (verified present in `/v3/api-docs`). The Constitution Check table above now carries **Pass** rows for both XVII and XVIII. No principle assessment changed from Pass to Fail or vice versa in the end state — the transient Fail/Partial readings were closed within this same amendment cycle, not left open.
