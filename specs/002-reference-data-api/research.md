...
**Decision**: The fixture is one checked-in JSON file, `src/main/resources/reference-data/appointment_titles.json`. It is generated once during implementation from the 194 distinct `name` values in the supplied AppointmentTitle reference extract. Only `name` is taken from the extract. Every other value is synthetic, following the rules in [data-model.md § Fixture generation rules](./data-model.md#fixture-generation-rules).
