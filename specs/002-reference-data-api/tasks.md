...
- [ ] T032 [US1] Generate `src/main/resources/reference-data/appointment_titles.json` once, following [data-model.md § Fixture generation rules](./data-model.md#fixture-generation-rules):
  - Read only the `name` column of the supplied AppointmentTitle reference extract (the AppointmentTitle CSV in `../ctam-jomockapi/joh-elinks-api/ReferenceData/`).
...
- [ ] T064 [P] Update `CLAUDE.md` § "Current repo state":
  ...
  - Correct § "Reference data": replace the claim that the data files are checked in with "Reference extracts (the E-Links Swagger PDF and reference-data files) are local-only reference material. They are gitignored and never committed, and the build doesn't depend on them." Don't name a path or any extract file name (finding F7).
